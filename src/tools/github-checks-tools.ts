...
export const createGitHubPullRequestChecksTool = (
  ...
  description: "Read GitHub check-run status for a commit or branch ref. This is read-only and does not require approval.",
  requiresApproval: false,
  async execute(input: GetPullRequestChecksInput, _context: ToolContext) {
    if (!input || typeof input !== "object") throw new Error("Pull request checks input is required");
