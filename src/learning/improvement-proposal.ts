...
export interface ImprovementProposal {
  ...
  title: string;
  rationale: string;
  evidence: { totalRecords: number; failures: number; successRate: number | null };
  status: "proposed";
}
...
export class DeterministicImprovementProposer implements ImprovementProposer {
  propose(summary: LearningSummary): ImprovementProposal[] {
    ...
    return [{
      ...
      evidence: { totalRecords: summary.total, failures: summary.failures, successRate: summary.successRate },
      status: "proposed",
    }];
  }
}
